...
Unload_Buf_LRU_CDC(long * out_despid_array, int max_n_batch, enum_t_vict suggest_type)
{
    ...
    if(suggest_type == ENUM_B_Any)
    {
        ...
        {
            paul_warning("Order to evict any cache block, but one of them has exhausted in advance.");
        }
    ...
    else if (suggest_type == ENUM_B_Dirty)
    {
        if(lru_dirty_ctrl.last_self_lru < 0)
            paul_warning("Order to evict [dirty] cache block, but one of them has exhausted in advance.");
    ...
    else if (suggest_type == ENUM_B_Clean)
    {
        if(lru_clean_ctrl.last_self_lru < 0)
            paul_warning("Order to evict [clean] cache block, but one of them has exhausted in advance.");
        goto FLAG_EVICT_CLEAN;
    }
    else
    {
        paul_warning("Order to evict [Unknown] type block.");
    }
